...
const renderPage = (template, page, locale) => {
  ...
  html = replaceFirst(html, /<div id="root"><\/div>/, `<div id="root"><main id="static-content" class="sr-only"><p>SAN Solution</p><h1>${escapeHtml(content.h1)}</h1><p>${escapeHtml(content.description)}</p><nav aria-label="Public pages"><a href="${locale === "id" ? "/solusi" : "/en/solutions"}">${locale === "id" ? "Solusi" : "Solutions"}</a> <a href="${locale === "id" ? "/harga" : "/en/pricing"}">${locale === "id" ? "Harga" : "Pricing"}</a> <a href="${locale === "id" ? "/kontak" : "/en/contact"}">${locale === "id" ? "Kontak" : "Contact"}</a></nav></main></div>`);
